...
def cli(
    ...
    # microbiomedata/mixs
    # Chris and Mark's mixs-source
    # contents of nmdc-schema?
    ...
    logger.warning("post submissions_frame")

    submissions_frame.to_csv(submission_frame_tsv, sep="\t", index=False)

    logger.warning("metadata_dict empty?!")
    logger.warning(metadata_dict)
...
def get_study_and_samples(url, params, cookies, known_orcids_tsv):
    ...
    # logger.warning(submissions_list[0].keys())
    ...
    # logger.warning(submissions_list[0][inner_key].keys())
    # was
    # # # ['template', 'studyForm', 'sampleData', 'multiOmicsForm'
    # now
    #     ['template', 'studyForm', 'sampleData', 'packageName', 'multiOmicsForm']
    # package name is new
    #
...
def lol_to_validatable(
    ...
    logger.warning("Makefile's previous submission = cc498964-d1da-416d-b353-aecf5f6c749d may not be present any more")
    logger.warning('current submission from study_id')
    logger.warning(study_id)
    logger.warning("mark's latest submission = 3e919e97-0437-4ca7-a758-ac36c45b908d")
    logger.warning("but metadata_dict from get_study_and_samples is empty")
    logger.warning(metadata_dict)
    # changes to response structure?
    # change to api arguments?
    # am I not paginating correctly?
    # where is api docs page?
    ...
    if study_id in known_templates:
        ...
        col_order = get_known_col_names(
            view=dh_view, selected_class_name=known_template
        )
